...
<script type="text/javascript">
	...
	function goPage(pcnt){
  	...
  	function checkPermissionAndRegister() {
  	    ...
  	    var deptRole = ${deptRole}; // JSP에서 부서 권한을 가져옵니다.

  	    if (userRole == 9) { 
  	        // userRole이 9이면 deptRole 관계없이 접근 가능
  	        location.href = 'insertDefect_type?lang=${param.lang}';
  	    } else if (userRole < 2) { 
  	        // 관리자 권한이 아닐 경우
...
<div class="container">
	<h2 class="text-center">불량유형 리스트</h2>
	<form id="frm01" class="form"  method="post">
		<input type="hidden" name="curPage" value="${sch.curPage}"/>
		<input type="hidden" name="pageSize" value="${sch.pageSize}"/>
<nav class="navbar navbar-expand-sm bg-dark navbar-dark">
	    <input placeholder="불량유형" name="dfc_typ" value="${param.dfc_typ}" class="form-control mr-sm-2" />    
...
private int dfc_typ_id;//불량유형아이디
   ...
   -->
    ...
    <tbody>
    	<c:forEach var="dt" items="${dlist}">
    	<tr ondblclick="goDetail('${dt.dfc_typ_id}')">
    	<td>${dt.dfc_typ_id}</td>
    	<td>${dt.dfc_typ}</td>
    	</tr>
    	</c:forEach>
  ...
  <!-- Scroll to Top Button-->
      ...
      <div class="modal fade" id="logoutModal" tabindex="-1" role="dialog"
		...
		<!-- End of Footer -->
				<script type="text/javascript">
		function goDetail(dfc_typ_id){
			var userRole = ${userRole}; // JSP에서 사용자 권한을 가져옵니다.
			var deptRole = ${deptRole}; // JSP에서 사용자 권한을 가져옵니다.
			 if (userRole == 9) { 
		  	        // userRole이 9이면 deptRole 관계없이 접근 가능
		  	        location.href = "Defect_typeDetail?dfc_typ_id="+dfc_typ_id+"&lang=${param.lang}";
