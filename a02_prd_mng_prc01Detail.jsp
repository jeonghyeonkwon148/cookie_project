...
<script type="text/javascript">
	...
	$(document).ready(function(){
		$("[name=prc_id]").val("${mlist.prc_id}")
		$("#uptBtn").click(function(){
			if(confirm("수정하시겠습니까")){
				$("form").attr("action","updatePrd_mng_prc01");
				$("form").submit();
			}
		})
		$("#delBtn").click(function(){
			if(confirm("삭제하시겠습니까")){
				$("form").attr("action","deletePrd_mng_prc01");
				$("form").submit();
			}
		})
		$("#repBtn").click(function(){
		if(confirm("다음 공정을 등록하시겠습니까?")){
			/*
			...
			$("[name=content]").val("\n\n\n\n\n==== 이전글 ===\n"+content)
			*/
			$("form").attr("action","insertPrd_mng_prc01_02?lang=${param.lang}");
			$("form").submit();
...
<div class="container">
	<h2 class="text-center">공정별 생산 관리 상세</h2>
	<form method="post">
	<div class="input-group mb-3">	
		<div class="input-group-prepend ">
			<span class="input-group-text  justify-content-center">공정별 생산 아이디</span>
		</div>
		<input name="prd_id_prc" class="form-control"  value="${mlist.prd_id_prc}" readonly/>	
	</div>				
	<div class="input-group mb-3">	
		<div class="input-group-prepend ">
			<span class="input-group-text  justify-content-center">이전 생산 아이디</span>
		</div>
		<input name="prvprd_id_prc" class="form-control"  value="${mlist.prvprd_id_prc}" readonly/>	
	</div>				
	<div class="input-group mb-3">	
		<div class="input-group-prepend ">
			<span class="input-group-text  justify-content-center">원재료 사용량</span>
		</div>
		<input name="prd_rwmt_usg" class="form-control"   value="${mlist.prd_rwmt_usg}"   />	
	</div>		
	<div class="input-group mb-3">	
		<div class="input-group-prepend ">
			<span class="input-group-text  justify-content-center">공정 생산량</span>
		</div>
		<input name="prd_yld" class="form-control"   value="${mlist.prd_yld}"   />	
	</div>		
	...
	<div class="input-group mb-3">	
		...
		<input name="prdc_dateStr" type="date" class="form-control" 
		  value="<fmt:formatDate 
			value='${mlist.prdc_date}' pattern='yyyy-MM-dd'/>"   />	
	</div>	
	<div class="input-group mb-3">	
		<div class="input-group-prepend ">
	...
	<div style="text-align:right;">
			<input type="button" class="btn btn-warning" value="다음생산공정" id="repBtn"/>
