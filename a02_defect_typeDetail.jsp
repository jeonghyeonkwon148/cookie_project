...
<script type="text/javascript">
	...
	if(msg!=""){
		...
		if(msg.indexOf("삭제성공")>=0){
			location.href='Defect_typeList?lang=${param.lang}'
		}
	...
	$(document).ready(function(){
		$("#uptBtn").click(function(){
			if(confirm("수정하시겠습니까")){
				$("form").attr("action","updateDefect_type");
				$("form").submit();
			}
		})
		$("#delBtn").click(function(){
			if(confirm("삭제하시겠습니까")){
				$("form").attr("action","deleteDefect_type");
				$("form").submit();
			}
		})		
	});
...
<div class="container">
	<h2 class="text-center">불량유형 상세</h2>
	<form method="post">
	<div class="input-group mb-3">	
		<div class="input-group-prepend ">
			<span class="input-group-text  justify-content-center">불량유형아이디</span>
		</div>
		<input name="dfc_typ_id" class="form-control"  value="${dlist.dfc_typ_id}" readonly/>	
	</div>			
	<div class="input-group mb-3">	
		<div class="input-group-prepend ">
			<span class="input-group-text  justify-content-center">불량유형</span>
		</div>
		<input name="dfc_typ" class="form-control"   value="${dlist.dfc_typ}"   />	
	</div>			
	<div style="text-align:right;">
			...
			<input type="button" class="btn btn-info" 
				onclick="location.href='Defect_typeList?lang=${param.lang}'" 
