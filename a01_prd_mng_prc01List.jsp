  	...
  	function checkPermissionAndRegister() {
		...
		var deptRole = ${deptRole}; // JSP에서 사용자 권한을 가져옵니다.
		 if (userRole == 9) { 
	  	        ...
	  	        location.href = 'insertPrd_mng_prc01?lang=${param.lang}';
	  	    } else
		if (userRole < 2) { // 관리자 권한이 아닐 경우
			...
			location.href = 'insertPrd_mng_prc01?lang=${param.lang}'; // 권한이 있을 경우 등록 페이지로 이동
		}
	}
</script>
</head>
...
<div class="container">
	<h2 class="text-center">공정별 생산리스트</h2>
 ...
 <table class="table table-hover table-striped">
   ...
   -->
    ...
    <tbody>
    	...
    	<tr ondblclick="goDetail('${mt.prd_id_prc}')">
    	<td>${mt.prd_id_prc}</td>
    	<td>${mt.prc_nm}</td>
    	<td>${mt.prvprd_id_prc}</td>
    	<td>${mt.prd_rwmt_usg}</td>
    	<td>${mt.prd_yld}</td>
    	<td>${mt.prv_prd_yld}</td>
    	<td><fmt:formatDate value="${mt.prdc_date}"/></td>
    	</tr>
    	</c:forEach>
    </tbody>
	</table> 
  ...
  <!-- Scroll to Top Button-->
      ...
      <div class="modal fade" id="logoutModal" tabindex="-1" role="dialog"
		...
		<script type="text/javascript">
		function goDetail(prd_id_prc){
			var userRole = ${userRole}; // JSP에서 사용자 권한을 가져옵니다.
			var deptRole = ${deptRole}; // JSP에서 사용자 권한을 가져옵니다.
			 if (userRole == 9) { 
		  	        // userRole이 9이면 deptRole 관계없이 접근 가능
		  	        location.href = "Prd_mng_prc01Detail?prd_id_prc="+prd_id_prc+"&lang=${param.lang}";
		  	    } else
			if (userRole < 2) { // 관리자 권한이 아닐 경우
				alert("권한이 없습니다. 등록할 수 없습니다.");
				return false; // 클릭 이벤트 방지
