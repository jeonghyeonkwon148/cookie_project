...
<script type="text/javascript">
	...
	$(document).ready(function(){
		$("[name=order_no]").val("${wlist.order_no}")
		
		$("#uptBtn").click(function(){
			if(confirm("수정하시겠습니까")){
				$("form").attr("action","updateWork_order");
				$("form").submit();
			}
		})
		$("#delBtn").click(function(){
			if(confirm("삭제하시겠습니까")){
				$("form").attr("action","deleteWork_order");
				$("form").submit();
			}
		})		
	});
...
<div class="container">
	<h2 class="text-center">작업지시서 상세</h2>
	<form method="post">
	<div class="input-group mb-3">	
		<div class="input-group-prepend ">
			<span class="input-group-text  justify-content-center">작업지시 아이디</span>
		</div>
		<input name="workl_id" class="form-control"  value="${wlist.workl_id}" readonly/>	
	</div>			
	<div class="input-group mb-3">	
		<div class="input-group-prepend ">
			<span class="input-group-text  justify-content-center">일일생산량(목표)</span>
		</div>
		<input name="day_pro" class="form-control"   value="${wlist.day_pro}"   />	
	</div>							
	<div class="input-group mb-3">	
		<div class="input-group-prepend ">
			<span class="input-group-text  justify-content-center">최종작업예정일</span>
		</div>
		<input name="final_schdlStr" type="date" class="form-control" 
		  value="<fmt:formatDate 
			value='${wlist.final_schdl}' pattern='yyyy-MM-dd'/>"   />	
	</div>	
	<div class="input-group mb-3">	
		<div class="input-group-prepend ">
