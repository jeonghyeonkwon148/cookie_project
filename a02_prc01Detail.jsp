...
<script type="text/javascript">
	...
	$(document).ready(function(){
		$("[name=mnum]").val("${plist.mnum}")
		$("[name=mch_id]").val("${plist.mch_id}")
		$("#uptBtn").click(function(){
			if(confirm("수정하시겠습니까")){
				$("form").attr("action","updatePrc01");
				$("form").submit();
			}
		})
...
<div class="container">
	<h2 class="text-center">생산공정 상세</h2>
	<form method="post">
	<div class="input-group mb-3">	
		<div class="input-group-prepend ">
			<span class="input-group-text  justify-content-center">공정아이디</span>
		</div>
		<input name="prc_id" class="form-control"  value="${plist.prc_id}" readonly/>	
	</div>			
	<div class="input-group mb-3">	
		<div class="input-group-prepend ">
			<span class="input-group-text  justify-content-center">공정명</span>
		</div>
		<input name="prc_nm" class="form-control"   value="${plist.prc_nm}"   />	
	</div>		
	<div class="input-group mb-3">	
		<div class="input-group-prepend ">
			<span class="input-group-text  justify-content-center">공정내용</span>
		</div>
		<input name="prc_dtl" class="form-control"   value="${plist.prc_dtl}"   />	
	</div>		
	<div class="input-group mb-3">	
		<div class="input-group-prepend ">
			<span class="input-group-text  justify-content-center">기준 생산량</span>
		</div>
		<input name="prc_bsln_qntt" class="form-control"   value="${plist.prc_bsln_qntt}"   />	
	</div>		
	<div class="input-group mb-3">	
		<div class="input-group-prepend ">
			<span class="input-group-text  justify-content-center">기준원재료량</span>
		</div>
		<input name="prc_stnd_qntt" class="form-control"   value="${plist.prc_stnd_qntt}"   />	
	</div>		
	<div class="input-group mb-3">	
		<div class="input-group-prepend ">
			<span class="input-group-text  justify-content-center">공정 시간</span>
		</div>
		<input name="ptime" class="form-control"   value="${plist.ptime}"   />	
	</div>		
