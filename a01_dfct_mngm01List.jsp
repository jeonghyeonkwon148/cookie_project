  	...
  	function checkPermissionAndRegister() {
		...
		var deptRole = ${deptRole}; // JSP에서 사용자 권한을 가져옵니다.
		 if (userRole == 9) { 
	  	        ...
	  	        location.href ='insertDfct_mngm01?lang=${param.lang}';
	  	    } else
		if (userRole < 2) { // 관리자 권한이 아닐 경우
			...
			location.href = 'insertDfct_mngm01?lang=${param.lang}'; // 권한이 있을 경우 등록 페이지로 이동
		}
	}
</script>
</head>
...
<div class="container">
	<h2 class="text-center">불량처리 결과</h2>
   ...
   -->
    ...
    <tbody>
    	...
    	<tr ondblclick="goDetail('${ft.dfct_id}')">
    	<td>${ft.dfct_id}</td>
    	<td>${ft.prc_nm}</td>
    	<td>${ft.dfct_pic_nm}</td>
    	<td>${ft.dfct_hndl_type}</td>
    	<td>${ft.dfct_prcs_rslt}</td>
    	<td><fmt:formatDate value="${ft.dfct_dsps_date}"/></td>
    	</tr>
    	</c:forEach>
    </tbody>
	</table> 
  ...
  <!-- Scroll to Top Button-->
      ...
      <div class="modal fade" id="logoutModal" tabindex="-1" role="dialog"
		...
		<script type="text/javascript">
		function goDetail(dfct_id){
			var userRole = ${userRole}; // JSP에서 사용자 권한을 가져옵니다.
			var deptRole = ${deptRole}; // JSP에서 사용자 권한을 가져옵니다.
			 if (userRole == 9) { 
		  	        // userRole이 9이면 deptRole 관계없이 접근 가능
		  	        location.href = "Dfct_mngm01Detail?dfct_id="+dfct_id+"&lang=${param.lang}";
		  	    } else
			if (userRole < 2) { // 관리자 권한이 아닐 경우
				alert("권한이 없습니다. 등록할 수 없습니다.");
				return false; // 클릭 이벤트 방지
