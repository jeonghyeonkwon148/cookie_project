  	...
  	function checkPermissionAndRegister() {
		...
		var deptRole = ${deptRole}; // JSP에서 사용자 권한을 가져옵니다.
		 if (userRole == 9) { 
	  	        ...
	  	        location.href = 'insertPeriodic_inspection?lang=${param.lang}';
	  	    } else
		if (userRole < 2) { // 관리자 권한이 아닐 경우
			...
			location.href = 'insertPeriodic_inspection?lang=${param.lang}'; // 권한이 있을 경우 등록 페이지로 이동
		}
	}
</script>
</head>
...
<table class="table table-hover table-striped">
   ...
   -->
    ...
    <tbody>
    	...
    	<tr ondblclick="goDetail('${pt.prd_ins_id}')">
    	<td>${pt.prd_ins_id}</td>
    	<td>${pt.mch_nm}</td>
    	<td>${pt.prd_ins_itm}</td>
    	<td>${pt.prd_unc}</td>
    	<td><fmt:formatDate value="${pt.prd_ins_dt}"/></td>
    	</tr>
    	</c:forEach>
    </tbody>
</table>
  ...
  <!-- Scroll to Top Button-->
      ...
      <div class="modal fade" id="logoutModal" tabindex="-1" role="dialog"
		...
		<script type="text/javascript">
		function goDetail(prd_ins_id){
			var userRole = ${userRole}; // JSP에서 사용자 권한을 가져옵니다.
			var deptRole = ${deptRole}; // JSP에서 사용자 권한을 가져옵니다.
			 if (userRole == 9) { 
		  	        // userRole이 9이면 deptRole 관계없이 접근 가능
		  	        location.href = "Periodic_inspectionDetail?prd_ins_id="+prd_ins_id+"&lang=${param.lang}";
		  	    } else
			if (userRole < 2) { // 관리자 권한이 아닐 경우
				alert("권한이 없습니다. 등록할 수 없습니다.");
				return false; // 클릭 이벤트 방지
